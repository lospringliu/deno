...
function setup () {
	...
	runtime.watch(
		() => state.price,
		(value, oldValue) => {
			console.log(`price = ${state.price}, changed from ${oldValue} to ${value}`)
	...
	runtime.watch(
		() => state.quantity,
		(value, oldValue) => {
			console.log(`quantity = ${state.quantity}, changed from ${oldValue} to ${value}`)
			console.log(`total: ${state.total}`)
			console.log("                     ---------------------")
		}
	)
...
setInterval(() => { reactive.state.price += 10; reactive.state.quantity += 5 }, 1000)
